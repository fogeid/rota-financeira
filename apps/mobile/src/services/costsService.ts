import { api } from './api';
import type { CostItem, CostsSummary } from '../types/api';

export const costsService = {
  async list(params?: { month?: string; type?: string; page?: number }): Promise<CostItem[]> {
    const { data } = await api.get<CostItem[]>('/costs', { params });
    return data;
  },
